			        ...
			        <div class="col-12">
				        ...
				        <table id="example" class="table table-responsive table-hover">
					        ...
					        <?php if(!empty($rows)){
					            foreach($rows as $row){
			                ?>
					            <tr id="row-<?php echo $row['cat_id'] ?>">
					        	    <td class="modelId"  width="10%"><?php echo $row['cat_id']; ?></td>
						            <td class="modelName" width="20%"><?php echo $row['category_name']; ?></td>
						            ...
						            <td  width="10%"><a href="javascript:void(0)" onclick="showEditForm(<?php echo $row['cat_id']; ?>)" class="btn btn-primary"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
  						            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456l-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  						            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
						            </svg> Edit</a></td>
						            <td  width="10%"><a href="#" onclick="confirmDeleteModel(<?php echo $row['cat_id']; ?>)" class="btn btn-danger"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
...
function categoryFormValidation(){
  //error count
  var count=0;
  var category = document.forms["categoryForm"]["category_name"].value;
  ...
  if(count > 0){
    ...
    $('#categoryForm').show();
    $.ajax({
        url : "<?php echo base_url();?>CManageKzCategory/updateCategory",
        type : 'POST',
        ...
        success : function(response){
            ...
            var cat_id=response['row']['cat_id'];
            $("#row-"+cat_id+" .modelName").html(response['row']['category_name']);
            $("#row-"+cat_id+" .modelCreated").html(response['row']['created_date']);
            $("#row-"+cat_id+" .modelModified").html(response['row']['modified_date']);
            $("#row-"+cat_id+" .modelStatus").html((response['row']['status'])==1? 'Active' : 'Inactive');
...
function deleteNow(){
    ...
    $.ajax({
        url : "<?php echo base_url();?>CManageKzCategory/deleteCategory/"+id,
        type : 'POST',
        data : '',
        dataType : 'json',
        success : function(response){
